...
由于对于这些指令的 print 的参数，其实是我们可控的 。所以，我们可以通过 `\r` 之类的字符来 rewind 回到行首 然后对命令执行掩盖和去除， 用户可能会略感到异常但也只是异常。尤其是 debug 模式没有开启的时候，用户看到命令的同时基本就已经被执行了。

此外我们还可以使用 `#` 来进行 shell 的注释，从而消除对于这类字符被 shell 解释器执行的可能

### 危害与影响

不只是 Open 计算器。我们几乎可以做任何操作 包括但不限于下载木马文件进行执行 并且由于教程中含有通过环境变量的值的方法传入各种 Token 所以也存在使用 curl 或者其他方法 向黑客的恶意 Webhook 服务器发送带有生产或是测试环境的相关的配置从而造成更大的危害。
...
> 如果支持 http 拉取远程 yaml 进行执行 或者 nacos 配置分发等，那么利用和演示就更加简单了。
